...
def level_lookup(df_name, level_col, lookup_col):
    ...
    idx = last_index.lookup(last_index.index, df_name[level_col] - 1)
    return pd.DataFrame({lookup_col: df_name.reindex(idx)[lookup_col].values}, index = df_name.index)


# BOM dosyasını okuma aşaması
bom_df = pd.read_excel("test_app/bom.xlsx", sheet_name = "2100 Sonrası BOM")
# relevant_columns = ["Ana Malzeme", "Bileşen Malzeme", "Seviye_Yeni", "    Miktar", "Açıklama"]
